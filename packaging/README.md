...
acceptance only after app writes its derived acknowledgment. Cold-start requests retry
until callback readiness; no-argument CLI sends its caller cwd instead of becoming a
Dock launch. Requests and acknowledgments are removed on success, rejection, signal,
or timeout. Symlink it only after placing
